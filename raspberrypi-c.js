    ...
    all.localClone(PREBUILT_SDK_REPO, PREBUILT_FOLDER, args.verbose, function (error) {
      ...
      if (process.platform == 'win32') {
        if (!all.fileExistsSync(TOOLCHAIN_ZIP_FILE) || !all.folderExistsSync(TOOLCHAIN_UNZIP_FOLDER)) {
          all.downloadAndUnzip('https://releases.linaro.org/14.09/components/toolchain/binaries/gcc-linaro-arm-linux-gnueabihf-4.9-2014.09_win32.zip',
            TOOLCHAIN_ZIP_FILE, all.getToolsFolder(), cb);
        }
        else {
        ...
        var cmds = [
          'sudo wget --output-document=' + all.getToolsFolder() + '/linaro.tar.xz'
