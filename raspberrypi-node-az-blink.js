...
function initTasks(gulp) {
    ...
    iotHubClient.open()
      ...
      .then(function (partitionIds) {
        return partitionIds.map(function (partitionId) {
          return iotHubClient.createReceiver('$Default', partitionId, { 'startAfterTime': Date.now() - 10000 }).then(function (receiver) {
  ...
  gulp.task('run-with-az-func', function () {
    // TODO: az command has changed to 'az storage account show-connection-string' in latest packages.
    // We need to update command here when using latest AZ bits.
